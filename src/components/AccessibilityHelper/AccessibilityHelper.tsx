...
import { useAccessibility, type FontScale } from "../../providers/accessibility";
...
function AccessibilityHelper() {
  const { isDarkMode, setIsDarkMode, fontScale, setFontScale } = useAccessibility();
      ...
      >
        <div className="accessibility-helper__section">
            ...
            <div className="accessibility-helper__toggle-group" role="group">
              {FONT_SCALE_OPTIONS.map((option) => (
                <button
                  key={option.id}
                  type="button"
                  className={`accessibility-helper__button ${
          ...
          <div className="accessibility-helper__status">
            <span>
              현재: {isDarkMode ? "다크" : "라이트"} · {currentFontScaleLabel}
            </span>
          </div>
        </div>
      </div>
