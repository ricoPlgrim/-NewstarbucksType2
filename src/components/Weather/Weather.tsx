 ...
 * @property {string} [apiKey] - OpenWeatherMap API 키 (환경 변수 VITE_WEATHER_API_KEY 없을 경우 사용)
 ...
 * 
 * ### 환경 변수 사용
 * .env 파일에 `VITE_WEATHER_API_KEY=your-api-key`를 설정하면
 ...
 * @example
 * // 환경 변수 사용
 * // .env: VITE_WEATHER_API_KEY=your-api-key
...
const Weather = ({ city = "Seoul", apiKey, useMock = true, useGPS = false, className = "" }: WeatherProps) => {
   ...
   * 2. 없으면 환경 변수 VITE_WEATHER_API_KEY 사용
   * 3. 둘 다 없으면 undefined (목업 모드로 동작)
   */
  const weatherApiKey = apiKey || import.meta.env.VITE_WEATHER_API_KEY;
